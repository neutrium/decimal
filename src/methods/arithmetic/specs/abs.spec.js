...
describe("Neutrium Decimal Absolute Value Tests", () => {
	...
	describe("abs() value", () => {
		...
		it.each([
			...
			['2817', '-2817'],
			['44535', '-44535'],
		])('case %#: should return the absolute value %s', (expected, input) => {
			expect(new Decimal(input).abs().toValue()).toEqual(expected);
		});
	});

	describe("Decimal abs value with exponential value", () => {
		...
		])('case %#: should return the absolute value of exponential %s', (expected, input) => {
			expect(new Decimal(input).abs().toValue()).toEqual(expected);
		});
	});
});