...
//
export function divide(x, y, pr, rm, dp, base) {
    const config = Decimal.config;
    // Set defaults where optional parameters not provided
    // Assumes non-zero x and k, and hence non-zero result.
    function multiplyInteger(x, k, base) {
        let temp, carry = 0, i = x.length;
        for (x = x.slice(); i--;) {
            temp = x[i] * k + carry;
            x[i] = temp % base | 0;
            carry = temp / base | 0;
    ...
    let cmp, e, i, k, logBase, more, prod, prodL, q, qd, rem, remL, rem0, sd, t, xi, xL, yd0, yL, yz, sign = x.s == y.s ? 1 : -1, xd = x.d, yd = y.d;
